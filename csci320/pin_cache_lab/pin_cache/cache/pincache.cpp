 ...
 *
 * $Id: pin-cache.cc 85 2012-09-13 20:33:07Z andse541 $
 */
...
KNOB<string> knob_policy(KNOB_MODE_WRITEONCE, "pintool", "replace",
...
plotFini(INT32 code, VOID *v)
{
        ...
        out << " " << ((100.0 * misses) / accesses)  << endl;
...
int main(int argc, char *argv[])
{
	...
	avdc_policy_t policy = 0;
	if(!strncmp(knob_policy.Value().c_str(),"RANDOM", 10))
		policy = 1;
	else if(!strncmp(knob_policy.Value().c_str(), "FIFO", 10))
		policy = 2;
	else
		policy = 0;


        avdc = avdc_new(size, block_size, assoc, policy);
        if (!avdc) {
        ...
        PIN_AddFiniFunction(plotFini, 0);
