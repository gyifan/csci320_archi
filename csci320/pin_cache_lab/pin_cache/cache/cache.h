...
typedef unsigned avdc_policy_t;
...
typedef struct {
	...
	// Used to FIFO policy one index each set.
	int*		index;
...
int avdc_resize(avdark_cache_t *self, avdc_size_t size,
		avdc_block_size_t block_size, avdc_assoc_t assoc, avdc_policy_t policy);
